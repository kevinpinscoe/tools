...
Bash script that names the terminal tab the calling process is running in. Intended for an AI agent working a Jira or YouTrack ticket: label the tab with the ticket key while the work is in flight, and re-label it with that key plus a lowercase `c` (`FLDW-12c`) once the ticket is closed. The `c` is a tab-label convention only — the key in YouTrack or Jira never changes.
...
- `--reset` is **not** the ticket-close move — that is the `c` suffix above. It remains for a tab with no finished ticket to remember. It unsets the tab's `automatic-rename` option instead of hardcoding `"zsh"`, handing naming back to tmux (global `automatic-rename` is `on`, format `#{pane_current_command}`). It therefore reverts to whatever is *running* — `zsh` at an idle shell, `claude` during an AI session. `zsh` is the idle case, not the reset case. It only names the tab explicitly if `automatic-rename` is off globally too.
- **Required by directive**, not optional: `when-creating-a-youtrack-ticket.md` §7 (name the tab when `Status` moves to `In Progress`) and §9 (rename to `<KEY>c` at `Done`/`Wont do`), and `when-in-work-context.md` instruction 8 for Jira. Both require it to be skipped silently where `~/tools` is unsynced or there is no tmux.
