...
- `claude-settings-baseline.sh` brings `~/.claude/settings.json` up to the Claude Code baseline: `permissions.defaultMode` = `auto`, and every skill in `~/.claude/skills` set to `user-invocable-only` in `skillOverrides`, so no skill is loaded into Claude's context until it is run with `/name` or via skills-tui. Fill-only (never changes a key already set) and idempotent, so it is safe to re-run on every shell start; mac-container does exactly that. Requires `jq` >= 1.6; runs under macOS's bash 3.2.
- `create-ticket-in-youtrack` / `create-ticket-in-youtrack.py` interactively creates a YouTrack issue in either `Work - Inbox` or `Kevin - Inbox`, setting the `Issue domain` custom field from the same "Is this work" answer that picks the project (`Employer work` for work, `Personal` otherwise) and always assigning the issue to Kevin Inscoe; reads the server URL from `$YOUTRACK_SERVER` and the `app/youtrack/work` API token from OpenBao (via parzival when the wrapper's `youtrack-work` AppRole is provisioned, `bao` directly otherwise). Prints the human-readable issue ID and a clickable issue URL on success.
...
- `human-only-search-guard` (invoked as `rg` or `ugrep` via symlinks of the same name) prompts `Are you human? Only a human is authorized to run this command.` before running the real binary; a non-interactive caller (an AI agent's Bash tool, a sandboxed exec, a pipe) has no TTY to answer with and is denied outright. Exists to stop unscoped recursive searches an AI session might otherwise run against the FLDW's spinning `/home` disk — see `~/ai/directives/unscoped-recursive-searches.md`.
- `gitcf` is a Python TUI that lists every untracked, modified or deleted file in the current git repo, lets you multi-select via an urwid checkbox picker, then commits (one batch commit if a memo is given, one per file otherwise) and pushes `HEAD` to `origin`. Every commit message is prefixed with `Committed by gitcf tool:` and a trailing space — both the auto-generated `Added`/`Modified`/`Deleted <name>` messages and any memo you type; pass `--no-prefix` to suppress it for that run. Deletions are listed so that a plain-`mv` rename can be committed whole — the untracked new name and the not-yet-staged deletion of the old name are both offered. A rename/copy git has already detected (`R`/`C` codes) is shown and committed as `Renamed <old> → <new>` / `Copied <old> → <new>` instead of the generic `Modified <name>`. Unresolved merge conflicts are withheld from the picker and named on stderr, since `git add` on one would stage the conflict markers. A selected change that is already recorded in the index — a staged deletion above all, and equally a staged modification, addition or rename — is committed without being re-`git add`ed, because a staged deletion's path is absent from both the working tree and the index, so asking git to add it fails outright. Files that a `pre-commit` hook has already swept into an earlier commit in the same run are skipped rather than retried — see the `gitcf` troubleshooting section in `RUNBOOK.md`. `CHECKPOINT.md` is withheld from the picker outright — it must never be committed — and its presence at the repo root prints a one-line warning before the picker opens. gitcf has no branch awareness of its own, so it always prints which branch is about to receive the commits/push before opening the picker, plus a reminder to log commits on the matching YouTrack issue when that branch looks like a project branch (`KDA-10`, `KEVIN-91`, etc.). It also fetches and compares the branch against its upstream before the picker opens, and warns — advisory only, never blocking — when the branch has fallen behind or diverged, since committing onto a stale branch is what makes the eventual push fail; pass `--no-fetch` to skip that check, and note that an unreachable origin is reported but never fatal. A rejected push is explained rather than raised: gitcf prints the commits the run made, the ahead/behind counts, and the `git pull --rebase` remedy, then exits non-zero without a traceback. A repo listed in `~/.config/gitcf/ignore.txt` is refused outright — no `git status`, no picker — for a repo another system owns writes to (e.g. `~/Projects/private/pim-records`, Radicale's own auto-committed history).
- `json-stats` reports statistics about a JSON file — modified date/time, file size, line count, number of top-level JSON documents, counts of each JSON data type, whether the JSON is nested, maximum nesting depth, and the size and path of the largest object/array — as a human-readable report by default, or raw JSON with `-j`. Handles NDJSON-style files holding several concatenated top-level JSON documents by aggregating stats across all of them. Requires `jq`.
...
- `reset` is a `$PATH` shim for the `reset` terminal command: it runs `set-ghostty-tab-name --reset` (best-effort — skipped silently if the command is missing or the process is outside tmux) to relabel the tab back to its idle name before `exec`-ing the real, OS-installed `reset` found elsewhere on `$PATH`. Same shim pattern as `human-only-search-guard` (`rg`/`ugrep`): resolves the real binary by walking `$PATH` and skipping its own directory, so it works regardless of where `reset` is actually installed.
