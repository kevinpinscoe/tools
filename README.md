...
- `what-did-i` / `what-did-i-accomplish-today.py` queries today's git commits from GitHub (via `gh`) and Gitea (via REST API) and writes a dated Markdown summary to `~/Journal/Personal Journal/ACCOMPLISHMENTS/YYYY-MM-DD/git-work-for-YYYY-MM-DD.md`; also prints to stdout.
