...
- `json-stats` reports statistics about a JSON file — line count, number of top-level JSON documents, counts of each JSON data type, whether the JSON is nested, maximum nesting depth, and the largest object/array — as a human-readable report by default, or raw JSON with `-j`. Handles NDJSON-style files holding several concatenated top-level JSON documents by aggregating stats across all of them. Requires `jq`.
