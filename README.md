...
- `myclaude` launches `claude` inside a named `abduco` session with `script` disk logging, and writes a cleaned `.txt` sibling next to the raw `.log` when the session exits (or via `myclaude --clean <log-file>` after a detach). Logs land at `<LOG_ROOT>/CLAUDE/_<REL>/<timestamp>.log`, where `_<REL>` encodes the cwd `myclaude` was launched from (e.g. `_.environment`, `_tools`, `_Projects-foo`, `_home`). `script` begins capturing immediately; `date && exec claude` runs inside that logging shell. Exports `CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN=1` so Claude Code renders into the terminal's native scrollback for cleaner logs. Detach with Ctrl+\; reattach with `abduco -a <session-name>`. Requires `abduco` and `util-linux-script` (`sudo dnf install util-linux-script`), plus Claude Code >= 2.1.132.
